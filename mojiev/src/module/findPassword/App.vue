  ...
  <div id="app">
    ...
    <link rel="stylesheet" :href="proPath+'/static/js/need/layer.css'">
      ...
      <ul class="header">
        ...
        <li class="header-middle text-c"><i>找回密码</i></li>
      ...
      <form class="main-form check-phone-form" v-if="step === 1">
        <ul class="form-list content-block">
          <li class="list-item"><label>手机号</label><input type="text" name="phonenumber" value="" placeholder="请输入手机号" maxlength="11" minlength="11" @change="checkForm"></li>
        </ul>
      ...
      <form class="main-form check-code-form" v-if="step === 3">
        ...
        <input type="button" value="下一步" class="form-submit" v-if="checkPass" @click="UserResetPassword"/>
        <input type="button" value="下一步" class="form-submit diabled" v-if="!checkPass" />
...
<script>
  ...
  export default {
    ...
    methods: {                          //方法
      ...
      checkPhone(){
        var that = this;
        var url = config.indexAjax.forgetpassword;
      ...
      ValidateBind(){
        var that = this;
        var url = config.indexAjax.validateforgetpassword;
        var phonenumber = that.phonenumber;
        var code = $('input[name="code"]').val();
        var params = {
          value:phonenumber,
      ...
      UserResetPassword(){
        var that = this;
        var url = config.indexAjax.resetpassword;
        var phonenumber = that.phonenumber;
        var password = $('input[name="password"]').val();
        var params = {
          password:password,
