...
<div class="container">
  <div class="content home" distance="55" v-pull-to-refresh="refresh" v-infinite-scroll="loadMore">
    ...
    <bar class="home-bar clearfix">
      <a class="tab-item " v-link="{path:'/home/man'}">
      ...
      <a class="tab-item active" v-link="{path: '/home/news'}">
...
export default {
  route: {
    data () {
      ...
      return this.$http.get(planPro.ajaxUrl.newsBT+'?start=1')
  ...
  methods: {
    refresh () {
      setTimeout(function () {
        this.pageIndex = 1
      this.$http.get(planPro.ajaxUrl.newsBT+'?start='+this.pageIndex)
      .then(({data: {success, info, data}}) => {
...
.shop-title img{height: 6.5rem;width: 100%;}
