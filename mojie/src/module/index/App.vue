    ...
    <v-scroll :on-refresh="refresh" :on-infinite="loadMore">
      ...
      <ul class="nav-tab clearfix">
        <li class="nav-tab-item" v-for="(mobileMenu,index) in allData.mobile_menu" >
          <a :href="index == 0?'videoMore.html':index == 1?'topicPage.html?price_rule=3':index == 2?'topicPage.html?price_rule=4':index == 3?'brandShop.html':mobileMenu.url" v-if="mobileMenu.url == ''">
            <div class="tab-img"><i class="sjq-icon"><img :src="mobileMenu.image"></i></div>
            <p>{{mobileMenu.text}}</p>
          </a>
          <a :href="mobileMenu.url" v-if="mobileMenu.url != ''">
            <div class="tab-img"><i class="sjq-icon"><img :src="mobileMenu.image"></i></div>
            <p>{{mobileMenu.text}}</p>
          </a>
        </li>
      ...
      <a :href="islogin == true?'redbagList.html?bonus_id='+bonusIds:'login.html'" class="img-mod clearfix" style="display:-none;">
        <img :src="proPath+'/static/images/active-reword-redbag.jpg'" alt="" style="float:left;">
      </a>
<!-- 
      <a href="http://www.sjq.cn/mojivue/module/miaoSha.html?topic_id=45" class="img-mod clearfix" >
        <img :src="proPath+'/static/images/action_banner_03.png'" alt="" style="float:left;">
      </a> -->
      ...
      <div class="content-block wh-recommend" style="margin-top:0;">
        <a href="login.html" class="img-mod" v-if="islogin == false && false">
          <img :src="proPath+'/static/images/active-reword-new.jpg'" alt="" style="float:left;">
        </a>
        <div class="block-title clearfix">
          <span class="m-title"><i class="sjqwx-icon wx-rem-icon"></i>网红推荐</span>
          <!-- <span class="o-title">拼团数量越多，价格越便宜</span> -->
          <a class="more" href="videoMore.html">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
        <slider-w-h :anchorRecommend="anchorRecommend"></slider-w-h>
      ...
      <!-- 明星同款 -->
      <div class="content-block mx-recommend">
        <div class="block-title clearfix">
          <span class="m-title"><i class="sjqwx-icon mx-re-icon"></i>明星同款</span>
          <!-- <span class="o-title">拼团数量越多，价格越便宜</span> -->
          <a class="more" href="topicPage.html?price_rule=3">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
        <!-- banner -->
        <div style="padding:0rem 0.896rem;">
          <slider-m-x :mxMarket="starSame"></slider-m-x>
        </div>
      ...
      <!-- 团划算 -->
      <div class="content-block mx-recommend" style="display:none">
        <div class="block-title clearfix">
          <span class="m-title">团划算</span>
          <span class="o-title">拼团数量越多，价格越便宜</span>
          <a class="more" href="market.html">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
        <ul class="mx-list clearfix">
      ...
      <div class="content-block xstp" style="display:none;">
        <div class="block-title clearfix">
          <span class="m-title"><i class="sjqwx-icon cx-re-icon"></i>团划算</span>
          <span class="o-title">拼团数量越多，价格越便宜</span>
          <a class="more" href="groupList.html">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
      ...
      <div class="content-block sjtm-recommend">
        <div class="block-title clearfix">
          <span class="m-title"><i class="sjqwx-icon cx-rem-icon"></i>四季特卖</span>
          <!-- <span class="o-title">拼团数量越多，价格越便宜</span> -->
          <a class="more" href="topicPage.html?price_rule=4">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
        <ul class="sjtm-list clearfix" >
            <li v-for="(factoryGood, index) in factoryGoods">
            <a :href="'groupDetail.html?goodsid='+factoryGood.id">
              <div class="clearfix">
                <img class="sjtm-img " :src="factoryGood.img.thumb" alt="">
              </div>
              
              <div class="sjtm-info">
                <p class="sjtm-name">{{factoryGood.goods_name}}</p>
                <p><span class="price">¥{{factoryGood.shop_price}}</span></p>
              </div>
            </a>
            </li>
      ...
      <div class="content-block pp-discount">
        <div class="block-title clearfix">
          ...
          <!-- <span class="o-title">拼团数量越多，价格越便宜</span> -->
          <a class="more" href="brandShop.html">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
      ...
      <!-- 批发名街 -->
      <div class="content-block mj-recommend">
        <div class="block-title clearfix">
          <span class="m-title"><i class="sjqwx-icon pf-rem-icon"></i>批发名街</span>
          <!-- <span class="o-title">拼团数量越多，价格越便宜</span> -->
          <a class="more" href="market.html">更多<i class="sjq-icon right-rem-icon"></i></a>
        </div>
        <!-- banner -->
        <slider-m-j :yunMarket="yunMarket"></slider-m-j>
        <!-- banner -->
      </div>
      <!-- 批发名街 -->
...
<script>
  ...
  import SliderMJ from 'components/SliderMJ'              //名街
  import SliderMX from 'components/SliderMX'              //明星同款
  import SliderWH from 'components/SliderWH'              //网红
  ...
  export default {
    ...
    mounted () {                        //页面完成加载
        ...
        _this.initBanner(function(data){
          ...
          setTimeout(function(){
            ...
            _this.initAnchorRecommend(_this.allData.anchor_recommend);
            



            _this.starSame = _this.allData.star_same;
            _this.factoryGoods = _this.allData.factory_goods;
          },500);
    ...
    methods: {                          //方法
      ...
      initAnchorRecommend(data_){
        var _this = this;
        var newArray = [];
        var count = parseInt(data_.length/4);
        if((data_.length%4)>0) count = count + 1;
        var i = 0
        for (var j = 0; j < count; j++) {
          var itemArray = [];
          for (var i = 0; i < 4; i++) {
            var newCount = j*4+i
            if(newCount<data_.length){
              itemArray.push(data_[newCount])
            }
          };
          newArray.push(itemArray);
        };
        // console.log(newArray);
        _this.anchorRecommend = newArray;
      },
      initSite(){
        //分站
        var _this = this;
      ...
      initActivityTopic(){
        ...
        that.activityTopic = activityTopicArr;
        if(typeof activityTopicArr[0].url != 'undefined'){
          that.bonusIds = activityTopicArr[0].url.split('&bonus_id=')[1];
        }
