      ...
      <div class="redboxlist">
        <ul class="myRedbox">
          <li v-for="item in sellerList" :class="colorstatus? 'useyes':'useno'">
            <div class="bLf">
              <p class="price"><label for="">￥</label><span>{{item.bonus_amount}}</span></p>
              <p class="messagePrice">{{item.bonus_name}}</p>
            </div>
            <div class="brt">
              <label>全场通用</label>
              <span class="time">使用期限</span>
        ...
        <div class="redKl">
        <p>
          <input type="text" placeholder="请输入红包口令密码">
          <span id="yz">验证</span>
        </p>
      </div>
    </div>
...
<script>
    ...
    methods: {                          //方法
        initEvent(){
            ...
            $("#yz").click(function(){
              var _thisDom = $(this);
              var val = _thisDom.siblings().val();
              _this.verificationRed(function(result){
        ...
        verificationRed(cb){
           var val = $("#yz").siblings().val();
           var params = {
                            bonus_password:val
                        };
           var url = config.indexAjax.BonusPassword;
